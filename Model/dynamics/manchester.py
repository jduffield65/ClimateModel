...
from Model.dynamics import lax_wendroff as lw
import matplotlib
...
if initial_conditions == UNIFORM_WESTERLY:
    ...
    height = 9750. + 1000. * np.exp(-((X - 0.25 * np.mean(x)) ** 2. + (Y - np.mean(y)) ** 2.)
                                    / (2. * std_blob ** 2.))
    ...
    height = 10250. - (max_wind_speed * f / g) * (Y - np.mean(y)) ** 2. / np.max(y) \
             - 1000. * np.exp(
        -(0.25 * (X - 1.5 * np.mean(x)) ** 2. + (Y - 0.5 * np.mean(y)) ** 2.) / (2. * std_blob ** 2.))
